...
from cache.redis_cache import PREFIX_SCREENER, get_json, set_json
...
def _screener_cache_key(params: dict) -> str:
    payload = json.dumps(params, sort_keys=True, default=str)
    digest = hashlib.sha256(payload.encode()).hexdigest()[:24]
    return f"{PREFIX_SCREENER}{digest}"
...
def screen_stocks(
    ...
    scan_limit: int = Query(default=2000, ge=50, le=10000),
    ...
    cache_key = _screener_cache_key(params)
    ...
    result = StockAnalysisResponse(items=items, total=total, scanned=scanned)
    set_json(cache_key, result.model_dump(mode="json"), 60)
    return result
