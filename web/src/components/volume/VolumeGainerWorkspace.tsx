...
function vsLonger(oneW: number | null, longer: number | null): number | null {
  if (oneW === null || longer === null || longer <= 0) return null;
  return ((oneW / longer) - 1) * 100;
}

export function VolumeGainerWorkspace() {
  ...
  const load = useCallback((next: Filters) => {
    startTransition(async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : "Failed to load volume gainers");
        setRows([]);
        setScanned(0);
      }
    });
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-1 text-sm text-[var(--ink-muted)]">
          Stocks where 1-week average volume is greater than 3-month, 6-month, and 1-year averages.
        </p>
      </header>
        ...
        <table className="w-full min-w-[980px] text-left text-sm">
          <thead className="border-b border-[var(--line)] text-[11px] uppercase tracking-[0.12em] text-[var(--ink-muted)]">
            <tr>
              ...
              <th className="px-3 py-3 font-medium">1W avg</th>
              <th className="px-3 py-3 font-medium">3M avg</th>
              <th className="px-3 py-3 font-medium">6M avg</th>
              <th className="px-3 py-3 font-medium">1Y avg</th>
              <th className="px-3 py-3 font-medium">vs 3M</th>
              <th className="px-3 py-3 font-medium">LTP</th>
              <th className="px-3 py-3 font-medium">Chg</th>
            </tr>
          </thead>
          <tbody>
            {pageRows.length === 0 && !pending ? (
              <tr>
                <td colSpan={10} className="px-3 py-10 text-center text-[var(--ink-muted)]">
                  No volume gainers match these filters.
                </td>
              </tr>
            ) : (
              pageRows.map((row, i) => {
                const oneW = row.avg_volume_1w;
                const vs3 = vsLonger(oneW, row.avg_volume_3m);
                return (
                  <tr
                    key={row.symbol}
                    className="border-b border-[var(--line)]/60 transition hover:bg-white/[0.02]"
                  >
                    ...
                    <td className="px-3 py-3 text-[var(--ink-soft)]">{row.sector || "—"}</td>
                    <td className="px-3 py-3 tabular-nums text-[var(--accent)]">
                      {formatCompact(oneW)}
                    </td>
                    <td className="px-3 py-3 tabular-nums">{formatCompact(row.avg_volume_3m)}</td>
                    <td className="px-3 py-3 tabular-nums">{formatCompact(row.avg_volume_6m)}</td>
                    <td className="px-3 py-3 tabular-nums">{formatCompact(row.avg_volume_1y)}</td>
                    <td className={cn("px-3 py-3 tabular-nums", tonePct(vs3))}>
                      {vs3 != null ? formatPct(vs3) : "—"}
                    </td>
                    <td className="px-3 py-3 tabular-nums">{formatNumber(row.ltp)}</td>
                    <td className={cn("px-3 py-3 tabular-nums", tonePct(row.change_pct))}>
